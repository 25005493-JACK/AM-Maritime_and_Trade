              ...
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    {email.automation && (email.automation.counts?.fields || 0) > 0 && (() => {
                      const auto = email.automation.counts?.auto_processed || 0;
                      const review = email.automation.counts?.flagged_for_review || 0;
                      const level = email.automation.level;
                      const tone = auto > 0 && review === 0
                        ? 'bg-emerald-950/60 text-emerald-300 border-emerald-700/60'
                        : auto > 0
                          ? 'bg-cyan-950/60 text-cyan-300 border-cyan-700/60'
                          : 'bg-amber-950/60 text-amber-300 border-amber-700/60';
                      const label = auto > 0 && review === 0
                        ? `AUTO-PROCESSED · L${level}`
                        : auto > 0
                          ? `PARTIAL · ${auto} auto / ${review} review · L${level}`
                          : `NEEDS YOU · L${level}`;
                      return (
                        <span
                          title={`${email.automation.level_label} - ${auto} field(s) auto-written, ${review} queued for review`}
                          className={`text-[10px] font-mono px-1.5 py-0.5 rounded border shrink-0 ${tone}`}
                        >
                          {label}
                        </span>
                      );
