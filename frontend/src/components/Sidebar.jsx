...
export default function Sidebar({ activeTab, setActiveTab, stats, onRefresh, theme, onToggleTheme }) {
  const navItems = [
    ...
    { id: 'trust', label: 'Trust & AI Controls', icon: ShieldCheck, badgeColor: 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30' },
      ...
      <div>
        ...
        <div className="p-4 border-b border-slate-800/80 flex items-center justify-between">
          ...
          <button
            ...
            className="p-2 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-cyan-400 border border-slate-800 transition"
          >
      ...
      <div className="p-4 border-t border-slate-800/80 bg-slate-900/40">
        <div className="flex items-center justify-between text-xs text-slate-400 mb-2">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-mono text-slate-300">FastAPI & React System</span>
          </div>
          <button 
            onClick={onRefresh}
        ...
        <div className="text-[11px] text-slate-500 font-mono">
          Theme: {theme === 'dark' ? 'Maritime Dark' : 'Enterprise Light'}
        </div>
      </div>
    </aside>
  );
