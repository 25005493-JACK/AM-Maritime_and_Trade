...
export default function SelfEvaluationView({ onRunSelfEvaluate, evaluationData, loading }) {
  ...
  useEffect(() => {
    if (!evaluationData && !localData && !loading) {
      setLocalLoading(true);
      fetch('/api/self-evaluate')
        .then((res) => (res.ok ? res.json() : Promise.reject('Failed')))
        .then((data) => setLocalData(data))
        .catch(() => setLocalData(null))
        .finally(() => setLocalLoading(false));
    }
  }, [evaluationData, localData, loading]);

  const activeData = evaluationData || localData;
  const report = activeData?.score_report;
  const isEvaluating = loading || localLoading;
      ...
      <div className="p-4 border-b border-blue-900/60 bg-slate-950 flex items-center justify-between shrink-0 shadow-md">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Autonomous scoring engine verifying all 520 inbox emails & attachments against Averis x Monash Hackathon standards.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <a
            ...
            className="px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-200 font-semibold text-xs flex items-center space-x-2 border border-blue-900/60 shadow-sm transition"
          >
          ...
          >
            ...
            <span>{isEvaluating ? 'Evaluating 520 Emails...' : 'Run Benchmark Evaluation'}</span>
          </button>
        </div>
      </div>
        ...
        {report ? (
          <>
            ...
            <div className="p-6 rounded-2xl bg-slate-900 border border-amber-500/50 shadow-xl flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="w-20 h-20 rounded-2xl bg-amber-500/20 border border-amber-500/50 text-amber-400 flex flex-col items-center justify-center font-mono font-extrabold text-3xl shadow-inner">
                  ...
                  <span className="text-[9px] font-sans text-amber-300 font-bold uppercase tracking-wider">FINAL SCORE</span>
                </div>
                <div>
                  ...
                  <div className="flex items-center space-x-4 mt-2.5 text-xs font-mono text-slate-400">
              ...
              <div className="grid grid-cols-3 gap-4 font-mono text-center">
                <div className="bg-slate-950/80 p-3.5 rounded-xl border border-blue-900/60">
            ...
            <div className="grid grid-cols-3 gap-5">
              ...
              <div className="rounded-xl border border-blue-900/60 bg-slate-900/80 p-5 space-y-3 shadow-lg">
                ...
                <div className="space-y-2 font-mono text-xs">
                  <div className="flex items-center justify-between p-2 rounded bg-emerald-950/40 border border-emerald-500/40 text-emerald-300">
                    <span>OK (No Mismatch)</span>
                    <strong className="text-sm">{report.scoreboard?.OK || 0}</strong>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded bg-rose-950/40 border border-rose-500/40 text-rose-300">
                    <span>MISMATCH (Defects Flagged)</span>
                    <strong className="text-sm">{report.scoreboard?.MISMATCH || 0}</strong>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded bg-amber-950/40 border border-amber-500/40 text-amber-300">
              ...
              <div className="rounded-xl border border-blue-900/60 bg-slate-900/80 p-5 space-y-3 shadow-lg">
                <div className="flex items-center justify-between border-b border-blue-900/60 pb-2">
                  ...
                  <span className="text-xs font-mono text-slate-500">Core Reasons</span>
                </div>
                <div className="space-y-2 font-mono text-xs">
                  {Object.entries(report.review_reasons_breakdown || {}).map(([reason, count]) => (
                    <div key={reason} className="flex items-center justify-between p-2 rounded bg-slate-950 border border-blue-900/40 text-slate-300">
              ...
              <div className="rounded-xl border border-blue-900/60 bg-slate-900/80 p-5 space-y-3 shadow-lg">
                ...
                <div className="space-y-1.5 font-mono text-xs max-h-48 overflow-y-auto">
                  {Object.entries(report.defect_field_counts || {}).map(([f, count]) => (
                    <div key={f} className="flex items-center justify-between p-1.5 rounded bg-slate-950 border border-blue-900/40 text-slate-300">
            ...
            <div className="rounded-xl border border-blue-900/60 bg-slate-900/80 p-5 space-y-3 shadow-lg">
              ...
              <div className="grid grid-cols-5 gap-3 font-mono text-center text-xs">
                {Object.entries(report.category_distribution || {}).map(([cat, count]) => (
                  <div key={cat} className="p-3 rounded-xl bg-slate-950 border border-blue-900/60">
          ...
          <div className="flex flex-col items-center justify-center py-24 text-slate-500">
            <Award className="w-16 h-16 stroke-[1.2] mb-3 text-slate-600 animate-pulse" />
            <h3 className="text-base font-semibold text-slate-300">Evaluating Test Data Benchmark Scoreboard...</h3>
            <p className="text-xs text-slate-500 max-w-md text-center mt-1">
              Click "Run Benchmark Evaluation" above to re-evaluate all 520 inbox emails & multi-format attachments.
            </p>
          </div>
        )}
      </div>
